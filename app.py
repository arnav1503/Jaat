...
def initialize_sheets_client():
    ...
    try:
        ...
        try:
            ...
            try:
                user_health_sheet = spreadsheet.add_worksheet(title="UserHealth", rows=500, cols=6)
                user_health_sheet.append_row(['UserId', 'Username', 'NutritionPoints', 'LastUpdated', 'BMI', 'Height'], value_input_option='USER_ENTERED')  # type: ignore  # type: ignore
...
def google_student_register():
    """Handles Google OAuth registration - extracts email and generates completion form."""
    try:
